...
def make_arrow(root, dataset_root):
    ...
    # json_object = json.dumps(counter)
    # Writing to sample.json
    # with open("counter_coco.json", "w") as outfile:
    #     outfile.write(json_object)
    # exit()
    ans2label = {k: i for i, k in enumerate(counter.keys())}

    # json_object = json.dumps(ans2label)
    # # Writing to sample.json
    # with open("ans2label_coco.json", "w") as outfile:
    #     outfile.write(json_object)

    label2ans = list(counter.keys())

    # json_object = json.dumps(label2ans)
    # # Writing to sample.json
    # with open("label2ans_coco.json", "w") as outfile:
    #     outfile.write(json_object)
    print('97')
